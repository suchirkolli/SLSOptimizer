...
run_button = st.sidebar.button("Run prediction & optimize (may take a few seconds)")
...
def run_pipeline_and_load(deficit_val):
    ...
    if not ref.empty:
        df = df.merge(ref[['district_id','pop_density','district_name','latitude','longitude']].drop_duplicates(), on='district_id', how='left')
...
if run_button:
    with st.spinner("Running predictions and optimizer..."):
        try:
...
with tab1:
    ...
    if forecasts.empty:
        ...
        # allow picking districts to plot
        ...
        if plot_df.empty:
            st.write("No forecast data for selected districts.")
        else:
            # If forecasts include time series, plot lines. If single snapshot, plot as bar/point.
            if plot_df['timestamp'].nunique() > 1:
                fig = px.line(plot_df, x='timestamp', y='predicted_peak_demand_mw', color='district_id',
                              labels={'predicted_peak_demand_mw':'Predicted Peak Demand (MW)'}, markers=True)
                fig.update_layout(height=450)
                st.plotly_chart(fig, use_container_width=True)
            else:
                # single timestamp: show bar chart of predicted values
        ...
        # Forecast vs Actual (for one selected district)
        ...
        if district_for_compare:
            # load modeling dataset for actuals
            if MODELING_DATA.exists():
                ...
                # forecasts may have time series; filter for district
                fc = forecasts[forecasts['district_id']==district_for_compare].sort_values('timestamp')
                fig2 = px.line()
                if not actual.empty:
                    # use peak_demand_mw or peak_demand_mw column as actual
                ...
                if not fc.empty:
                    if 'predicted_peak_demand_mw' in fc.columns:
                        ...
                        fig2.add_scatter(x=fc['timestamp'], y=fc.iloc[:,2], mode='markers', name='Predicted')  # fallback
                if fig2.data:
...
with tab2:
    ...
    if risks.empty:
        st.info("No outage risk data available. Run pipeline to compute risks.")
    else:
        map_df = risks.merge(district_ref, on='district_id', how='left') if not district_ref.empty else risks.copy()
        # If centroids available, build folium map
